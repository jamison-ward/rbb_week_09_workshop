...
```{r}
ggplot(data = ebola_wa,
       mapping = aes(x = date_of_hospitalisation)) +
...
ggplot(data = ebola_wa,
  ...
  geom_histogram(binwidth = 7,
                 color = "white",
                 fill = "darkgray") +
  labs(title = "Weekly Ebola Cases in West Africa",
...
ebola_wa_summary <- ebola_wa %>% 
  ...
  group_by(date_of_onset, outcome) %>% 
  count() %>% 
  ungroup()
...
```{r}
ggplot(data = ebola_wa_summary,
       mapping = aes(x = date_of_onset, y = n)) +
...
ggplot(data = ebola_wa_summary,
  ...
  scale_x_date(date_breaks = "2 months",
               date_labels = "%b '%y") +
  labs(title = "Ebola Cases in West Africa",
       subtitle = "Grouped by Case Outcome",
       x = "Date of Onset",
       y = "Number of Cases",
       color = "Outcome") +
  theme_minimal()
